...
function displayUrlsToAssets(body: string): string {
  // A data URI is the whole image in base64 — a megabyte for a screenshot. As a
  // regex pattern that size fails to compile in WebKit and the save silently
  // never happens, so this is a plain split/join.
  let result = body;
  for (const [dataUri, relPath] of dataUriToRelPath.entries()) {
    if (result.includes(dataUri)) result = result.split(dataUri).join(relPath);
  }
  return result;
}
