  ...
  return (
    <nav className={styles.root}>
      <div className={styles.header}>
        ...
        <button
          className={`${styles.syncButton} ${needsSync ? styles.syncButtonActive : ""}`}
          onClick={onSync}
      ...
      <div className={styles.section}>
        <NavItem icon={<HomeIcon />} label="All notes" active={activeView === "all"} onClick={() => onViewChange("all")} />
        <NavItem icon={<ClockIcon />} label="Recent" active={activeView === "recent"} onClick={() => onViewChange("recent")} />
        <NavItem icon={<CalendarIcon />} label="Today" active={false} onClick={onTodayNote} />
      </div>

      {/* Type views — only shown if the vault has typed notes */}
        ...
        <div className={styles.section}>
          ...
          {availableTypes.map((type) => (
            <NavItem
              ...
              active={activeView === `type:${type}`}
              onClick={() => onViewChange(`type:${type}`)}
            />
          ))}
        </div>
      )}
          ...
          {agentBranches.map((b) => (
            <div key={b.name} className={styles.agentBranch}>
              ...
              <div className={styles.agentActions}>
                ...
                <button className={styles.discardBtn} onClick={() => onDiscardBranch(b.name)}>Discard</button>
              </div>
            </div>
          ))}
        </div>
...
function GitSection({
  status, isDirty, changedCount, commits, onCommit,
}: {
  status: VaultStatus | null;
  isDirty: boolean;
  changedCount: number;
  ...
  return (
    <div className={styles.section}>
      <div className={styles.gitHeader}>
        ...
        {isDirty && (
          <button className={styles.expandToggle} onClick={() => setExpanded((x) => !x)}>
            {expanded ? "−" : "Commit"}
          </button>
        )}
      </div>
        ...
        <div className={styles.commitForm}>
          ...
          {status && (
            <div className={styles.changedFiles}>
              {[...status.staged, ...status.unstaged, ...status.untracked].slice(0, 5).map((f) => (
                <span key={f} className={styles.changedFile}>{f.split("/").pop()}</span>
              ))}
              {changedCount > 5 && <span className={styles.changedFile}>+{changedCount - 5} more</span>}
            </div>
          )}
...
function HomeIcon() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>;
}
function ClockIcon() {
...
function TypeIcon() {
  return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5L14.5 2z"/><polyline points="14 2 14 8 20 8"/></svg>;
}
...
function AgentIcon() {
  return <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/></svg>;
}
