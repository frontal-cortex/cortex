...
pub fn run_view(ctx: &AppCtx, spec: String) -> Result<ResolvedTable> {
...
pub fn list_collections(ctx: &AppCtx) -> Result<Vec<String>> {
    let root = ctx.vault.0.lock().unwrap().clone().ok_or(AppError::NoVault)?;
    let mut out = Vec::new();
...
pub fn run_chart(ctx: &AppCtx, spec: String) -> Result<ChartResult> {
    let root = ctx.vault.0.lock().unwrap().clone().ok_or(AppError::NoVault)?;
...
pub fn run_stats(ctx: &AppCtx, spec: String) -> Result<StatsResult> {
    let root = ctx.vault.0.lock().unwrap().clone().ok_or(AppError::NoVault)?;
...
pub fn set_cell(ctx: &AppCtx, source: String, row_id: String, field: String, value: String, ty: String) -> Result<()> {
    let root = ctx.vault.0.lock().unwrap().clone().ok_or(AppError::NoVault)?;
...
pub fn add_row(ctx: &AppCtx, source: String, id: String, fields: std::collections::HashMap<String, String>) -> Result<()> {
    let root = ctx.vault.0.lock().unwrap().clone().ok_or(AppError::NoVault)?;
...
pub fn export_to_file(ctx: &AppCtx, kind: String, target: String, dest: String) -> Result<()> {
    std::fs::write(&dest, export_text(ctx, kind, target)?)?;
    Ok(())
}

/// The exported document itself — what a served device downloads, having no
/// path on this machine to write to.
pub fn export_text(ctx: &AppCtx, kind: String, target: String) -> Result<String> {
    let root = ctx.vault.0.lock().unwrap().clone().ok_or(AppError::NoVault)?;
...
pub fn ensure_row(ctx: &AppCtx, source: String, id: String, fields: std::collections::HashMap<String, String>) -> Result<String> {
    let root = ctx.vault.0.lock().unwrap().clone().ok_or(AppError::NoVault)?;
    let fields: std::collections::BTreeMap<String, String> = fields.into_iter().collect();
...
pub fn list_row_templates(ctx: &AppCtx, source: String) -> Result<Vec<String>> {
    let root = ctx.vault.0.lock().unwrap().clone().ok_or(AppError::NoVault)?;
...
pub fn add_row_from_template(ctx: &AppCtx, source: String, id: String, template: String, fields: std::collections::HashMap<String, String>) -> Result<()> {
    let root = ctx.vault.0.lock().unwrap().clone().ok_or(AppError::NoVault)?;
    let fields: std::collections::BTreeMap<String, String> = fields.into_iter().collect();
...
pub fn save_row_as_template(ctx: &AppCtx, source: String, row_id: String, name: String) -> Result<()> {
...
pub fn duplicate_row(ctx: &AppCtx, source: String, row_id: String, new_id: String, created: String) -> Result<()> {
    let root = ctx.vault.0.lock().unwrap().clone().ok_or(AppError::NoVault)?;
...
pub fn delete_row(ctx: &AppCtx, source: String, row_id: String) -> Result<()> {
    if row_id.contains('/') || row_id.contains("..") {
        return Err(AppError::Other("Invalid row id".into()));
    }
    let root = ctx.vault.0.lock().unwrap().clone().ok_or(AppError::NoVault)?;
