...
| Backlinks panel | done | — | "Linked from" under the note (`BacklinksPanel.tsx`): each source note with the passage around every link to this one, the link set off, clipped at word boundaries (`note::link_contexts`, read from the index's FTS bodies, so nothing is written to disk). `cortex backlinks` and the MCP `backlinks` carry the same `mentions`. Hidden when empty. Not a separate pane; no unlinked mentions (below). |
